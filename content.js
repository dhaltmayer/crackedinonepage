window.onload = function(){
	...
	if ((domain == "www.cracked.com") || (domain == "cracked.com")){
		domain = "cracked";
	}
	else if ((domain == "www.diply.com") || (domain == "diply.com")){
		domain = "diply";
	}
	...
	if (domain == "cracked"){
	...
	else if (domain == "diply"){
	
		p2URL = theURL.substring(0, theURL.length) + "/2";
		p3URL = theURL.substring(0, theURL.length) + "/3";
		p4URL = theURL.substring(0, theURL.length) + "/4";
		p5URL = theURL.substring(0, theURL.length) + "/5";
		...
		var p1content = document.getElementsByClassName('card')[0];
		var p2content, p3content, p4content, p5content;

		request2.onreadystatechange=function(){
			if (request2.readyState==4 && request2.status==200){
				var page2 = request2.response;
				p2content = page2.getElementsByClassName('card')[0];
				var newbody = p1content.innerHTML + p2content.innerHTML;
			
				request3.onreadystatechange=function(){
					if (request3.readyState==4 && request3.status==200){
						var page3 = request3.response;
						p3content = page3.getElementsByClassName('card')[0];
						newbody = newbody + p3content.innerHTML;
					}
				}
				...
				request4.onreadystatechange=function(){
					if (request4.readyState==4 && request4.status==200){
						var page4 = request4.response;
						p4content = page4.getElementsByClassName('card')[0];
						newbody = newbody + p4content.innerHTML;
					}
				}
				...
				request5.onreadystatechange=function(){
					if (request5.readyState==4 && request5.status==200){
						var page5 = request5.response;
						p5content = page5.getElementsByClassName('card')[0];
						newbody = newbody + p5content.innerHTML;
					}
